...
| **node**                  | A pipeline graph vertex representing one phase execution at a specific revision cycle. E.g., `implement_0`, `verify_1`. Introduced in Phase 3.               | `vertex` (too academic)                                                 |
...
| **marker**                | A file written to `.smith/<task-slug>/` as evidence of a completed phase. E.g., `tested`, `reviewed`.                                                        | `flag`, `sentinel`                                                      |
...
| `vertex`               | Alternative for "node"       | Too academic — the graph abstraction is already sufficient                                  |
