...
**[enforced]** The reviewer agent must produce a `.smith/<task-slug>/reviewed` marker with `critical: 0` before the closer can create a PR. Critical findings (enforced principle violations, failing tests, missing test coverage for public API changes) block PR creation. Advisory findings are posted as PR comments.

Check: `SLUG=$(cat .smith/active | tr -d '[:space:]') && test -f ".smith/${SLUG}/reviewed" && grep -q "critical: 0" ".smith/${SLUG}/reviewed"`
