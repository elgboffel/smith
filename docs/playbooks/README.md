...
1. A human fills in a task template (from `tasks/templates/`) and drops it in the target repo's `.smith/tasks/active/`.
...
5. The closer opens a PR in the target repo (requires `.smith/<task-slug>/reviewed`).
6. After merge, the task JSON status is updated; runtime task files stay in ignored `.smith/` history.
